...
class Covid19RLlibEnvWrapper(MultiAgentEnv):
    """
    Environment wrapper for Covid19 -RLlib. It sub-classes MultiAgentEnv.
    This wrapper adds the action and observation space to the environment,
    and adapts the reset and step functions to run with RLlib.
    """
    ...
    def __init__(self, env_config, verbose=False):
        ...
        if self.env.world.federal_reserve.multi_action_mode:
            ...
            self.action_space_fed.dtype = np.int64
            self.action_space_fed.nvec = self.action_space_fed.nvec.astype(np.int64)
        ...
        if self.verbose:
            ...
            print("[EnvWrapper] Obs (p)   ")
            pretty_print(self.observation_space_pl)
            print("[EnvWrapper] Obs (f)   ")
            pretty_print(self.observation_space_fed)
            print("[EnvWrapper] Action (a)", self.action_space)
            print("[EnvWrapper] Action (p)", self.action_space_pl)
