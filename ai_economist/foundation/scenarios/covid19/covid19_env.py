...
class CovidAndEconomyEnvironment(BaseEnvironment):
    ...
    def scenario_step(self):
        ...
        if self.use_cuda:
            self.cuda_step(
                self.world.cuda_data_manager.device_data("susceptible"),
                self.world.cuda_data_manager.device_data("infected"),
                self.world.cuda_data_manager.device_data("recovered"),
                self.world.cuda_data_manager.device_data("deaths"),
                self.world.cuda_data_manager.device_data("vaccinated"),
                self.world.cuda_data_manager.device_data("unemployed"),
                self.world.cuda_data_manager.device_data("subsidy"),
                self.world.cuda_data_manager.device_data("productivity"),
                self.world.cuda_data_manager.device_data("stringency_level"),
                self.world.cuda_data_manager.device_data("num_stringency_levels"),
                self.world.cuda_data_manager.device_data("postsubsidy_productivity"),
                self.world.cuda_data_manager.device_data("num_vaccines_available_t"),
                self.world.cuda_data_manager.device_data("real_world_stringency_policy_history"), 
                self.world.cuda_data_manager.device_data("beta_delay"),
                self.world.cuda_data_manager.device_data("beta_slopes"),
                self.world.cuda_data_manager.device_data("beta_intercepts"),
                self.world.cuda_data_manager.device_data("beta"),
                self.world.cuda_data_manager.device_data("gamma"),
                self.world.cuda_data_manager.device_data("death_rate"),
                self.world.cuda_data_manager.device_data("incapacitated"),
                self.world.cuda_data_manager.device_data("cant_work"),
                self.world.cuda_data_manager.device_data("num_people_that_can_work"),
                self.world.cuda_data_manager.device_data("us_state_population"),
                self.world.cuda_data_manager.device_data("infection_too_sick_to_work_rate"),
                self.world.cuda_data_manager.device_data("population_between_age_18_65"),
                self.world.cuda_data_manager.device_data("filter_len"),
                self.world.cuda_data_manager.device_data("num_filters"),
                self.world.cuda_data_manager.device_data("delta_stringency_level"),
                self.world.cuda_data_manager.device_data("grouped_convolutional_filter_weights"),
                self.world.cuda_data_manager.device_data("unemp_conv_filters"),
                self.world.cuda_data_manager.device_data("unemployment_bias"),
                self.world.cuda_data_manager.device_data("signal"),
                self.world.cuda_data_manager.device_data("daily_production_per_worker"),
                self.world.cuda_data_manager.device_data("maximum_productivity"),
                self.world.cuda_data_manager.device_data(f"{_OBSERVATIONS}_a_world-agent_state"),
                self.world.cuda_data_manager.device_data(f"{_OBSERVATIONS}_a_world-agent_postsubsidy_productivity"),
                self.world.cuda_data_manager.device_data(f"{_OBSERVATIONS}_a_world-lagged_stringency_level"),
                self.world.cuda_data_manager.device_data(f"{_OBSERVATIONS}_a_time"),
                self.world.cuda_data_manager.device_data(f"{_OBSERVATIONS}_p_world-agent_state"),
                self.world.cuda_data_manager.device_data(f"{_OBSERVATIONS}_p_world-agent_postsubsidy_productivity"),
                self.world.cuda_data_manager.device_data(f"{_OBSERVATIONS}_p_world-lagged_stringency_level"),
                self.world.cuda_data_manager.device_data(f"{_OBSERVATIONS}_f_world-agent_state"),
                self.world.cuda_data_manager.device_data(f"{_OBSERVATIONS}_f_world-agent_state_FED_Balance_Sheet"),
                self.world.cuda_data_manager.device_data(f"{_OBSERVATIONS}_p_time"),
                self.world.cuda_data_manager.device_data(f"{_OBSERVATIONS}_f_time"),
                ...
                self.world.cuda_data_manager.device_data("MaxQE"), 
                self.world.cuda_data_manager.device_data("MoneySupply"), 
                self.world.cuda_data_manager.device_data("MaxMoneySupply"), 
                self.world.cuda_data_manager.device_data("FEDBalanceSheet"), 
                self.world.cuda_data_manager.device_data("MaxFEDBalanceSheet"), 
    ...
    def compute_reward(self):
        ...
        if self.use_cuda:
            self.cuda_compute_reward(
                self.world.cuda_data_manager.device_data(f"{_REWARDS}_a"),
                self.world.cuda_data_manager.device_data(f"{_REWARDS}_p"),
                self.world.cuda_data_manager.device_data(f"{_REWARDS}_f"),
                self.world.cuda_data_manager.device_data("num_days_in_an_year"),
                self.world.cuda_data_manager.device_data("value_of_life"),
                self.world.cuda_data_manager.device_data("economic_reward_crra_eta"),
                self.world.cuda_data_manager.device_data("min_marginal_agent_health_index"),
                self.world.cuda_data_manager.device_data("max_marginal_agent_health_index"),
                self.world.cuda_data_manager.device_data("min_marginal_agent_economic_index"),
                self.world.cuda_data_manager.device_data("max_marginal_agent_economic_index"),
                self.world.cuda_data_manager.device_data("min_marginal_planner_health_index"),
                self.world.cuda_data_manager.device_data("max_marginal_planner_health_index"),
                self.world.cuda_data_manager.device_data("min_marginal_planner_economic_index"),
                self.world.cuda_data_manager.device_data("max_marginal_planner_economic_index"),
                self.world.cuda_data_manager.device_data("weightage_on_marginal_agent_health_index"),
                self.world.cuda_data_manager.device_data("weightage_on_marginal_agent_economic_index"),
                self.world.cuda_data_manager.device_data("weightage_on_marginal_planner_health_index"),
                self.world.cuda_data_manager.device_data("weightage_on_marginal_planner_economic_index"),
                self.world.cuda_data_manager.device_data("agents_health_norm"),
                self.world.cuda_data_manager.device_data("agents_economic_norm"),
                self.world.cuda_data_manager.device_data("planner_health_norm"),
                self.world.cuda_data_manager.device_data("planner_economic_norm"),
                self.world.cuda_data_manager.device_data("deaths"),
                self.world.cuda_data_manager.device_data("subsidy"),
                self.world.cuda_data_manager.device_data("postsubsidy_productivity"),
                self.world.cuda_data_manager.device_data("_done_"),
                self.world.cuda_data_manager.device_data("_timestep_"),
                self.world.cuda_data_manager.meta_info("n_agents"),
                self.world.cuda_data_manager.meta_info("episode_length"),
                self.world.cuda_data_manager.device_data("QE"),  
                ...
                self.world.cuda_data_manager.device_data("MaxFEDBalanceSheet"), 
                self.world.cuda_data_manager.device_data("USDebt"), 
                self.world.cuda_data_manager.device_data("MaxUSDebt"), 
                block=self.world.cuda_function_manager.block,
                grid=self.world.cuda_function_manager.grid,
            )
