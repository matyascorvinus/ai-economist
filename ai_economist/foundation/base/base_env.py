...
class BaseEnvironment(ABC):
    ...
    def __init__(
        ...
        n_planners = 1
        # Foundation assumes there's only a federal reserve
        n_federal_reserve = 1
        
        self.num_agents = (
            n_agents + n_planners + n_federal_reserve
    ...
    def _generate_observations(self, flatten_observations=False, flatten_masks=False):
        ...
        agent_wise_federal_reserve_obs = {
            "f" + str(agent.idx): {} for agent in self.world.agents
        }
        ...
        for idx, o in world_obs.items():
            ...
            elif idx in agent_wise_federal_reserve_obs:
                agent_wise_federal_reserve_obs[idx].update(
                    {"world-" + k: v for k, v in o.items()}
                )
            else:
                raise KeyError
        ...
        for k, v in agent_wise_federal_reserve_obs.items():
            if len(v) > 0: 
